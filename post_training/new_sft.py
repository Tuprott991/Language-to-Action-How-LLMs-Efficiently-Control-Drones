...
# ---------------------------
# FIXED: Proper preprocessing function for causal LM
# ---------------------------
def preprocess_function(example):
    ...
    # Mask the prompt part (only train on assistant response)
    # First, find where the assistant response starts
...
lora_config = LoraConfig(
    ...
    target_modules=["q_proj", "k_proj", "v_proj", "o_proj", "up_proj", "down_proj", "gate_proj"],  # Fixed target modules
...
# Trainer
# ---------------------------
num_train_samples = int(0.3 * len(tokenized_datasets["train"]))
train_subset = tokenized_datasets["train"].select(range(num_train_samples))

num_eval_samples = int(0.5 * len(tokenized_datasets["validation"]))
